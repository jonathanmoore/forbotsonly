...
async function createProdigiOrderForOrder(orderId: string): Promise<string | null> {
  ...
  try {
    ...
    // Note: Address already validated and stored by fulfillPaidOrder when order entered awaiting_approval
    // No need to re-validate here - use the stored validated address
    ...
    // Build Prodigi address object from stored validated address, omitting line2 if empty/whitespace
...
const TOOL_DEFINITIONS = {
  ...
  add_to_cart: {
    name: 'add_to_cart',
    description: 'STEP 4: Add a product to your cart with size. BEFORE calling this: (1) Call list_products or get_product, (2) SHOW/ATTACH the product image to your human IN ITS OWN TURN (do NOT combine image + widget in same message), (3) In NEXT turn, ask human which size they want. THEN call this with their chosen size. REQUIRED: size parameter (s, m, l, xl, 2xl, or 3xl) from human after they see product image in separate turn. Your session mark (from identify_agent) is used automatically.',
    inputSchema: {
      type: 'object',
      properties: {
        productId: {
  ...
  switch (toolName) {
    ...
    case 'create_checkout': {
      ...
      return {
        ...
        next_step: 'BROWSER CHECKOUT CREATED (fallback path). For agent demonstrations, PREFER complete_payment with Link SPT instead: (1) Call native request_virtual_card (Cursor spend-approval UI, NOT a Link MCP tool) with amountCents 4000, merchantName "forbotsonly", merchantUrl from this response; (2) Wait for human approval; (3) Call Link MCP get_spend_request with include shared_payment_token to get SPT; (4) Call complete_payment with sharedPaymentToken + confirmed shippingAddress. If native request_virtual_card unavailable (platform limitation), human can complete browser checkout at checkoutUrl. After payment completes, order status becomes awaiting_approval (manual review before fulfillment). Call get_order with orderId to check status.',
      };
    }
...
After you attach the images and end your turn, IN YOUR NEXT TURN: Use Link MCP list_shipping_addresses to get saved address, confirm city/ZIP with human (US-only, never dump full street in public chat). Then AGENTS SHOULD call complete_payment (API path with Link SPT - preferred) OR create_checkout (browser fallback). AGENT PAYMENT: (1) Call native request_virtual_card (Cursor spend-approval UI, NOT Link MCP) with amountCents 4000; (2) Wait approval; (3) Call Link MCP get_spend_request to get SPT; (4) Call complete_payment with SPT + shippingAddress.`,
    ...
    case 'recover_paid_checkout': {
      ...
      if (!order) {
        ...
        // HARD RULE from #76: Extract size from Stripe metadata (NEVER hardcode)
        ...
        // HARD RULE from #76: Extract mark from Stripe metadata (NEVER default to hexagon/orange)
        ...
        // From #78: Call fulfillPaidOrder to validate address and mark as awaiting_approval
        // This ensures the order goes through the approval workflow instead of auto-creating Prodigi
        try {
          await fulfillPaidOrder(orderId, session);
          console.log(`[Recovery] Created order ${orderId} with mark (${markShape}, ${markColor}) size ${size.toUpperCase()} from metadata - status: awaiting_approval`);
        } catch (err: any) {
